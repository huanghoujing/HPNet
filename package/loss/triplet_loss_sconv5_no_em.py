...
from ..eval.torch_distance import compute_dist_with_qg_visibility
...
def construct_triplets(dist_mat, vis_sum, labels, hard_type='tri_hard'):
    ...
    is_invalid = vis_sum == 0
    dist_mat[is_pos & is_invalid] = - 10000
    dist_mat[is_neg & is_invalid] = 10000
...
# Triloss for concatenated parts, with visibility
class TripletLoss(Loss):
    ...
    def calculate(self, batch, pred, hard_type=None):
        """
        Args:
        ...
        """
        cfg = self.cfg
        feat = torch.stack(pred['tri_loss_em_list'], 1)
        vis = pred['visible']
        dist_mat, vis_sum = compute_dist_with_qg_visibility(feat, feat, vis, vis, dist_type='euclidean')
        if hard_type is None:
            hard_type = cfg.hard_type
        dist_ap, dist_an = construct_triplets(dist_mat, vis_sum, batch['label'], hard_type=hard_type)
    ...
    def __call__(self, batch, pred, step=0, hard_type=None):
        ...
        # Calculation
        cfg = self.cfg
        res = self.calculate(batch, pred, hard_type)
        loss, dist_an, dist_ap = res['loss'], res['dist_an'], res['dist_ap']

        # Meter
        if cfg.name not in self.meter_dict:
            self.meter_dict[cfg.name] = Meter(name=cfg.name, fmt='{:.1f}')
        self.meter_dict[cfg.name].update(loss.item())

        # precision
        key = 'prec'
        if key not in self.meter_dict:
            self.meter_dict[key] = Meter(name=key, fmt='{:.1%}')
        ...
        key = 'd_ap'
        if key not in self.meter_dict:
            self.meter_dict[key] = Meter(name=key, fmt='{:.1f}')
