...
class TripletLoss(Loss):
    ...
    def connect_type_gp(self, batch, pred, step=0, hard_type=None):
        res1 = self.calculate(pred['tri_loss_em_list'][0], batch['label'], hard_type=hard_type)
        return res1['loss'], res1['dist_an'], res1['dist_ap']

    def __call__(self, batch, pred, step=0, hard_type=None):

        # Calculation
        cfg = self.cfg
        loss, dist_an, dist_ap = getattr(self, cfg.connect_type)(batch, pred, step, hard_type)
