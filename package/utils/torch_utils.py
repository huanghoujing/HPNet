...
def rename_keys(ori_ckpt_path, new_ckpt_path, ori_key='bn_list.0.', new_key='bn.'):
    ckpt = torch.load(ori_ckpt_path, map_location=(lambda storage, loc: storage))
    sd = ckpt['state_dicts']['model']
    new_sd = {(k.replace(ori_key, new_key) if k.startswith(ori_key) else k): v for k, v in sd.items()}
    ckpt['state_dicts']['model'] = new_sd
    may_make_dir(osp.dirname(new_ckpt_path))
    torch.save(ckpt, new_ckpt_path)
    print('=> Rename keys of ckpt {} and save it to {}'.format(ori_ckpt_path, new_ckpt_path))
