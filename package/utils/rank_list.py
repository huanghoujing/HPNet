...
def save_rank_list_to_im(rank_list, q_im_path, g_im_paths, save_path, same_id=None, resize_h_w=(128, 64), root=None, query_save_path=None):
    ...
    """
    if root is not None:
        q_im_path = osp.join(root, q_im_path)
    query_im = read_im(q_im_path, convert_rgb=True, resize_h_w=resize_h_w, transpose=True)
    if query_save_path is None:
        ims = [query_im]
        ncol = len(rank_list) + 1
    else:
        save_im(query_im, query_save_path, transpose=True)
        ims = []
        ncol = len(rank_list)
    for i, ind in enumerate(rank_list):
        g_im_path = g_im_paths[ind]
        if root is not None:
            g_im_path = osp.join(root, g_im_path)
        im = read_im(g_im_path, convert_rgb=True, resize_h_w=resize_h_w, transpose=True)
        ...
        ims.append(im)
    im = make_im_grid(ims, 1, ncol, 8, 255)
