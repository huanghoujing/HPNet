...
from .np_distance import compute_dist_with_cfg
...
def get_scores_str_r1_r3(mAP, cmc_scores, score_prefix):
    return score_prefix + '[cmc1: {}], [cmc3: {}]'.format(score_str(cmc_scores[0]), score_str(cmc_scores[2]))
# get_scores_str = get_scores_str_r1_r3 # TODO:TMP
...
def eval_feat(dic, cfg):
    ...
    for i in range(num_chunks):
        ...
        dic_tmp = {'q_feat': dic['q_feat'][st_ind:end_ind], 'g_feat': dic['g_feat']}
        if 'q_visible' in dic:
            dic_tmp['q_visible'] = dic['q_visible'][st_ind:end_ind]
            dic_tmp['g_visible'] = dic['g_visible']
        dist_mat = compute_dist_with_cfg(dic_tmp, cfg)
